...
import { getFederation, mountFedifyRoutes } from "./fedify";
import { generateRSAKeyPair } from "./keys";
import type { Follow, Post, User } from "./models";
import {
	clearSessionCookie,
	sessionMiddleware,
	setSessionCookie,
        ...
        <ul class="timeline-list">
          ${allPosts
						.map((post) => {
							...
							const remoteDomain = isRemote
								? post.userId.toString().split("@")[1]
								: null;
...
app.get("/@*", async (c) => {
	...
	const followingCount = await follows.countDocuments({
		followerId: profileUser._id?.toString(),
	});
...
app.get("/user/:username", async (c) => {
	...
	if (profileUser._id) {
		postCount = await posts.countDocuments({
...
app.get("/post/:postId", async (c) => {
	...
	const postAuthor = await users.findOne({
		_id: new ObjectId(post.userId.toString()),
	});
	if (!postAuthor) {
	...
	const followerCount = await follows.countDocuments({
		followingId: postAuthor._id?.toString(),
	});
	const followingCount = await follows.countDocuments({
		followerId: postAuthor._id?.toString(),
	});
...
app.post("/setup", async (c) => {
	...
	setCookie(c, "session", result.insertedId.toString(), {
		httpOnly: true,
		path: "/",
	});
	return c.redirect("/");
...
app.get("/", async (c) => {
	...
	if (loggedIn && user) {
		const remoteFollows = await follows
			.find({
		...
		for (const remoteFollow of remoteFollows) {
			try {
						...
						if (outboxUrl) {
							...
							if (outboxResponse.ok) {
								...
								if (outbox.orderedItems) {
									for (const activity of outbox.orderedItems.slice(0, 5)) {
				...
				console.error(
					`Error fetching remote posts for ${remoteFollow.followingId}:`,
					error,
				);
			}
		}
...
app.get("/debug/users", async (c) => {
	...
	const allUsers = await users
		.find({}, { projection: { username: 1, name: 1, _id: 0 } })
		.toArray();
...
app.post("/remote-follow", async (c) => {
	...
	const body = await c.req.parseBody();
	const remoteUser =
		typeof body["remoteUser"] === "string" ? body["remoteUser"] : "";

	if (!remoteUser || !remoteUser.includes("@")) {
		return c.json({
			success: false,
			error:
				"Invalid remote user format. Use @username@domain or username@domain",
		});
	}

	// Handle both @username@domain and username@domain formats
	const cleanRemoteUser = remoteUser.startsWith("@")
		? remoteUser.slice(1)
		: remoteUser;
	...
	try {
		...
		try {
			...
			if (!remoteActor) {
				return c.json({
					success: false,
					error: `Could not find user ${remoteUser}`,
				});
			}
			...
			console.log(
				`Sending follow request from ${currentUser.username} to ${remoteUser}`,
			);
