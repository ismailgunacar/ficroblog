...
import { setSessionCookie, getSessionUser } from '../session';
...
export function createHomeRoutes(client: MongoClient) {
  ...
  app.get('/', async (c) => {
    ...
    const allPosts = await db.collection<Post>('posts')
      .find()
      .sort({ createdAt: -1 })
      .limit(50)
      .toArray();
  ...
  app.post('/', async (c) => {
    ...
    console.log('Form data - password exists:', !!password, 'content exists:', !!content);
    ...
    if (password) {
      ...
      if (user && await verifyPassword(password, user.passwordHash)) {
        ...
        if (isAjax) {
          const html = await generateHomeHTML(db, displayUser, domain, false, true);
...
async function generateHomeHTML(db: any, user: User | null, domain: string, loggedIn: boolean, invalidPassword: boolean): Promise<string> {
  if (!user) {
    user = await db.collection<User>('users').findOne();
  }

  const postCount = user ? await db.collection('posts').countDocuments({ userId: user._id }) : 0;
  const followerCount = user ? await db.collection('follows').countDocuments({ followingId: user._id }) : 0;
  const followingCount = user ? await db.collection('follows').countDocuments({ followerId: user._id }) : 0;
