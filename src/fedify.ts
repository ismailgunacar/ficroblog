...
export function createFederationInstance(mongoClient: MongoClient) {
	...
	const kvStore = new MongoDBKVStore(
		mongoClient,
		mongoClient.db().databaseName || "fongoblog2",
	);
	...
	federation.setOutboxDispatcher(
		...
		async (ctx, identifier, cursor) => {
			...
			return {
				items: activities,
				nextCursor:
					userPosts.length === limit ? (skip + limit).toString() : null,
			};
		},
	);
	...
	federation
		.setActorDispatcher("/users/{identifier}", async (ctx, identifier) => {
			...
			console.log(
				`🔑 Actor dispatcher: got ${keys.length} keys for ${identifier}`,
			);
		...
		.setKeyPairsDispatcher(async (ctx, identifier) => {
			console.log(
				`🔑 Key pairs dispatcher called for identifier: ${identifier}`,
			);
			const db = mongoClient.db(); // Use default database from connection string
			const users = db.collection("users");
			...
			let rsaKey = await keys.findOne({
				user_id: user._id,
				type: "RSASSA-PKCS1-v1_5",
			});
			let ed25519Key = await keys.findOne({
				user_id: user._id,
				type: "Ed25519",
			});
			...
			if (!rsaKey) {
				...
				console.log(
					`✅ RSA key generated and stored for user: ${user.username}`,
				);
			}
			...
			if (!ed25519Key) {
				...
				console.log(
					`✅ Ed25519 key generated and stored for user: ${user.username}`,
				);
			}
				...
				try {
					...
					if (rsaKey.privateKey && rsaKey.publicKey) {
						...
						const privateKey = await importJwk(
							JSON.parse(rsaKey.private_key),
							"RSASSA-PKCS1-v1_5",
						);
						const publicKey = await importJwk(
							JSON.parse(rsaKey.public_key),
							"RSASSA-PKCS1-v1_5",
						);
			...
			if (ed25519Key) {
				try {
					...
					if (ed25519Key.privateKey && ed25519Key.publicKey) {
						...
						// Otherwise, import from stored JWK
						const privateKey = await importJwk(
							JSON.parse(ed25519Key.private_key),
							"Ed25519",
						);
						const publicKey = await importJwk(
							JSON.parse(ed25519Key.public_key),
							"Ed25519",
						);
						keyPairs.push({ privateKey, publicKey });
						console.log(
							`✅ Successfully imported Ed25519 key pair from storage`,
						);
					}
				} catch (error) {
			...
			console.log(
				`🔑 Returning ${keyPairs.length} key pairs for user: ${user.username}`,
			);
			return keyPairs;
		});
	...
	federation
		...
		.on(Follow, async (ctx, follow) => {
			...
			console.log(
				`Received follow request from ${from.preferredUsername} to ${username}`,
			);
			...
			if (!existingFollow) {
				...
				console.log(
					`Created follow relationship: ${from.preferredUsername} -> ${username}`,
				);
			} else {
				console.log(
					`Follow relationship already exists: ${from.preferredUsername} -> ${username}`,
				);
			}
...
export function mountFedifyRoutes(app: Hono, mongoClient: MongoClient) {
	...
	app.use(
		"*",
		fedifyHonoMiddleware(federation, () => ({})),
	);
}
...
export function getFederation() {
	if (!globalFederation) {
		throw new Error(
			"Federation not initialized. Call mountFedifyRoutes first.",
		);
	}
