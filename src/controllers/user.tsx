...
import { Layout, PostList, PostPage, Profile } from "../views";
...
export async function handleProfilePage(c: any, username: string) {
  ...
  const rootPosts = postsWithActor.filter(
    (post: any) => !("reply_to" in post) || !post.reply_to,
  );

  return c.html(
    <Layout
      user={userWithActor as User & Actor}
...
export async function handlePostPage(c: any, username: string, postId: number) {
  console.log(
    `[DEBUG] handlePostPage called with username=${username}, postId=${postId}`,
  );
  await connectToDatabase();
  const usersCollection = getUsersCollection();
  const actorsCollection = getActorsCollection();
  ...
  const user = await usersCollection.findOne({ id: 1 });
  if (!user) {
    console.log("[DEBUG] handlePostPage: user not found (id=1)");
    return c.notFound();
  }
  const actor = await actorsCollection.findOne({ user_id: user.id });
  if (!actor) {
    console.log("[DEBUG] handlePostPage: actor not found for user id=1");
    return c.notFound();
  }
  const actorWithUsername = { ...actor, username: user.username };
  console.log(
    `[DEBUG] handlePostPage: looking up post with id=${postId}, actor_id=${actor.id}`,
  );

  // First, let's check what posts exist for this actor
  const allPosts = await postsCollection.find({ actor_id: actor.id }).toArray();
  console.log(
    `[DEBUG] handlePostPage: found ${allPosts.length} posts for actor_id=${actor.id}`,
  );
  console.log(
    `[DEBUG] handlePostPage: post IDs: ${allPosts.map((p) => p.id).join(", ")}`,
  );
  ...
  if (!post) {
    console.log(
      `[DEBUG] handlePostPage: post not found (id=${postId}, actor_id=${actor.id})`,
    );
    return c.notFound();
  }

  console.log(
    `[DEBUG] handlePostPage: found post with id=${post.id}, content="${post.content.substring(0, 50)}..."`,
  );
  ...
  if (currentUserSession && currentUserSession.username) {
    const currentUserDb = await usersCollection.findOne({
      username: currentUserSession.username,
    });
    if (currentUserDb) {
      const currentActor = await actorsCollection.findOne({
        user_id: currentUserDb.id,
      });
      if (currentActor) currentActorId = currentActor.id;
    }
  }

  // Create post object with actor fields but preserve post.id
  const postWithActor = {
    ...post,
    uri: actorWithUsername.uri,
    handle: actorWithUsername.handle,
  ...
  console.log(
    `[DEBUG] handlePostPage: final post object has id=${postWithActor.id}`,
  );

  return c.html(
    <Layout
      user={{ ...user, ...actorWithUsername }}
      isAuthenticated={isAuthenticated}
    >
      <PostPage
        name={actorWithUsername.name ?? user.username}
...
export async function serveActivityPubPost(c: any) {
  ...
  const note = {
    ...
    ...(post.reply_to
      ? {
          inReplyTo:
            post.reply_to && typeof post.reply_to === "number"
              ? `https://${domain}/${username}/posts/${post.reply_to}`
              : post.reply_to,
        }
      : {}),
